...
本阶段只定义四份输入文件的解析结果和领域模型，不实现代码，不进行设备布局、物流布线、仿真或蓝图导出。
...
| 种植机（液体模式） | 5 | 5 | sisisisisi | sososososo | nfnn | nnnnn | 20 | 基础生产 |
| 废水处理机 | 3 | 3 | nnn | nnn | nfin | nnn | 50 | 基础生产 |
| 装备原件机 | 6 | 4 | sisisisisisi | sososososo | nnnn | nnnn | 10 | 合成制造 |
| 灌装机 | 6 | 4 | sisisisisisi | sososososososo | nnnn | nnnn | 20 | 合成制造 |
| 灌装机（液体模式） | 6 | 4 | sisisisisisi | sososososososo | nfinn | nnnn | 20 | 合成制造 |
| 灌装机（气体模式） | 6 | 4 | sisisisisisi | sososososososo | nginn | nnnn | 20 | 合成制造 |
| 封装机 | 6 | 4 | sisisisisisi | sososososo | nnnn | nnnn | 20 | 合成制造 |
| 研磨机 | 6 | 4 | sisisisisisi | sososososo | nnnn | nnnn | 50 | 合成制造 |
...
| 拆解机（液体模式） | 6 | 4 | sisisisisisi | sososososo | nnnn | nnfon | 20 | 合成制造 |
| 拆解机（气体模式） | 6 | 4 | sisisisisisi | sososososo | nnnn | nngon | 20 | 合成制造 |
...
配方实体必须保留双输入、双输出、副产物、环境和催化剂，不能只保存“主输入/主输出”。已确认：催化剂列是每次配方的固定消耗量；物品单位为个，液体和气体单位为滴；理论速率是 `数量 * 60 / 时间`。
...
- 人工逐行抽查设备表，核对尺寸、接口串、耗电功率和类别。
- 用 `sisisi`、`nfinfin`、`nnnnn`、`nsisisisisisisin` 做接口分词样例核对。
- 抽查单输入/双输入、单输出/双输出、环境和催化剂配方，核对单位与源行号。
- 抽查 6 个发电实体，核对燃烧时间、功率和低容谷地电池 20 秒/220kW 示例。
- 将每个歧义点作为 Schema 评审问题，在确认前禁止转化为硬编码规则。

## 9. 下一步建议

下一步仍然只做设计：基于本 M0 文档编写 JSON Schema 草案和“原始值/规范值/来源引用/未决状态”的字段约定，并为设备接口、旋转映射、配方物料类别和规则元数据设计评审用例。Schema 获得确认后，才进入解析器实现；解析器完成并通过样例核对后，再进入独立规则验证器。